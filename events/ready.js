...
module.exports = async (client) => {
  ...
  setInterval(async () => {
    ...
    const guilds = await Guild.find({ 'channels.apps.id': { $in: Object.keys(appUpdates) } });
      ...
      for (const guild of guilds) {
        for (const channel of guild.channels) {
          ...
          for (const branchName in app.branches) {
            ...
            if (!channel.apps.some(app => app.id === appID && app.branches.includes(branchName))) {
              continue;
            }
            ...
            embeds.push(new EmbedBuilder()
              ...
              .addFields(
                { name: 'Build ID', value: branch.buildid || 'Unknown', inline: true, },
                { name: 'Last Updated', value: branch.timeupdated ? `<t:${branch.timeupdated}:f>` : 'Never', inline: true },
                { name: 'Password Required', value: branch.pwdrequired ? 'Yes' : 'No', inline: true },
                { name: 'Description', value: branch.description || 'None', inline: false })
              .setTimestamp()
          ...
          for (let i = 0; i < embeds.length; i += 10) {
            await client.channels.cache.get(channel.id).send({ embeds: embeds.slice(i, i + 10) });
          }
        }
      }
    }
