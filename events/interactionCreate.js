    ...
    if (interaction.isChatInputCommand()) {
		...
		switch (interaction.customId) {
			case 'game-search-options': {
				...
				if (!guild.channels.some(channel => channel.id == interaction.channelId)) {
					guild.channels.push({ id: interaction.channelId, apps: [] });
				}

				let channel = guild.channels.find(channel => channel.id == interaction.channelId);
				if (!channel.apps.some(app => app.id == appID)) {
					...
					await interaction.editReply({ content: 'This game is already being tracked in this channel.', components: [] });
				...
				let embed = new EmbedBuilder()
					...
					.setAuthor({ name: 'SteamDB', iconURL: 'https://steamdb.info/static/logos/512px.png', url: 'https://steamdb.info/' })
					.setThumbnail(`https://cdn.cloudflare.steamstatic.com/steam/apps/${appID}/capsule_sm_120.jpg?t=${Date.now()}`)
					.addFields(
						{ name: 'Name', value: appName, inline: true, },
						{ name: 'App ID', value: appID, inline: true },
					)
					.setTimestamp()
					.setFooter({ text: 'SteamDB', iconURL: 'https://steamdb.info/static/logos/512px.png' });
			...
			case 'branch-select-options': {
				...
				let values = interaction.values.map(value => value.slice(value.indexOf('-') + 1));
