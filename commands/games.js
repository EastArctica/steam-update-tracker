...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand => 
            subcommand
                ...
                .addStringOption(option => option.setName('game').setDescription('The name of the game to add')
                    .setRequired(true)))
        .addSubcommand(subcommand =>
            subcommand
                .setName('remove')
                .setDescription('Remove a monitored game')
                .addStringOption(option => option.setName('game').setDescription('The name of the game to remove')
                    .setRequired(true)
    ...
    async execute(interaction) {
        ...
        if (!this[subcommand]) {
            interaction.reply('We\'re sorry. An error occured while executing your command. 🙁');
        } else {
    ...
    async remove(interaction) {
        ...
        let guild = await Guild.findOne({ id: interaction.guildId, 'channels.id': interaction.channelId });
        
        // If we don't have any apps for the current channel, there's nothing to be removed.
        let channel;
        if (!guild || !(channel = guild.channels.find(channel => channel.id == interaction.channelId)) || channel.apps.length == 0) {
            await interaction.reply({ content: 'Looks like you don\'t have any games being monitored in this channel.', ephemeral: false });
            return;
        }


        // If we don't have the game they asked for... well... It can't be removed.
        let removedGame = channel.apps.find(app => app.name == interaction.options.getString('game'));
        if (!removedGame) {
            await interaction.reply({ content: 'You\'re not currently tracking that game.', ephemeral: false });
            return;
        }
        ...
        await interaction.reply({ content: `Successfully removed \`${removedGame.name}\` \`(${removedGame.id})\``, ephemeral: false });
    },
    async autocomplete(interaction) {
        switch (interaction.options.getSubcommand()) {
            ...
            case 'remove':
                ...
                await interaction.respond(results.map(result => ({ name: result.item.name, value: result.item.name })));
                break;
            default:
    ...
    async manage(interaction) {
        ...
        if (!guild || !(channel = guild.channels.find(channel => channel.id == interaction.channelId)) || channel.apps.length == 0) {
            ...
            await interaction.editReply({ content: 'Looks like you don\'t have any games being monitored in this channel.' });
            return;
        }

        // If we don't have the game they asked for... well... It can't be managed.
        let channelApp = channel.apps.find(app => app.name == interaction.options.getString('game'));
        if (!channelApp) {
            // TODO: Update to embed
            await interaction.editReply({ content: 'You\'re not currently tracking that game.' });
            return;
        }
        ...
        const gameOptions = new StringSelectMenuBuilder()
            ...
            .addOptions(...[...app.branches.keys()].map(branch => new StringSelectMenuOptionBuilder()
                .setLabel(branch)
